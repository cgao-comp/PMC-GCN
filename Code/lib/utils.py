...
def generate_dataset(data, seq_len, pre_len, time_len=None, split_ratio=0.8, normalize=True):
    ...
    return np.array(train_X), np.array(train_Y), \
        np.array(test_X), np.array(test_Y), max_val


def generate_torch_datasets(data, seq_len, pre_len, time_len=None, split_ratio=0.8, normalize=True):
    train_X, train_Y, test_X, test_Y, max_val = generate_dataset(data, seq_len, pre_len, time_len=time_len,
                                                        split_ratio=split_ratio, normalize=normalize)
    # train_dataset = torch.utils.data.TensorDataset(, )
    train_x_tensor = torch.FloatTensor(train_X)
    train_target_tensor = torch.FloatTensor(train_Y)
    ...
    test_x_tesor = torch.FloatTensor(test_X)
    test_target_tensor = torch.FloatTensor(test_Y)
    test_x_tesor = test_x_tesor.unsqueeze(1)
    test_x_tesor = test_x_tesor.permute(0, 3, 1, 2)
    test_target_tensor = test_target_tensor.permute(0, 2, 1)
    test_dataset = torch.utils.data.TensorDataset(test_x_tesor, test_target_tensor)
    return test_dataset, train_dataset, train_x_tensor, train_target_tensor, test_x_tesor, test_target_tensor, max_val
