...
from Model.GCN_models import chev_GCN, GCN
from layers import calculate_laplacian_with_self_loop
...
class STransformer(nn.Module):
    def __init__(self, embed_size, heads, adj, cheb_K, dropout, device, forward_expansion):
...
#         self.embed_liner = nn.Linear(adj.shape[0], embed_size)
        ...
        # 调用chev_GCN
        # self.gcn = chev_GCN(embed_size, embed_size*2, embed_size, adj, device, cheb_K, dropout)
        # 调用 EST-GCN中使用的GCN embed_size = input_dim; embed_size * 2 = hidden_dim; embed_size = output_dim
        self.gcn = GCN(adj, embed_size, embed_size)
        # 邻接矩阵归一化（ESTGCN方式）
...
class ES_GCN(nn.Module):
    ...
    def forward(self, value, key, query):
        ...
        # hidden_states = self.dropout(x_fft)
        # query = self.norm3(hidden_states + query1)
        ##
...
#         D_T = self.temporal_embedding(torch.arange(0, T).to('cuda:0'))    # temporal embedding选用nn.Embedding
        ...
        # 全连接层, (去掉前面注意力后,用后面两行全连接替换)
        forward = self.feed_forward(query)
        out = self.norm2(forward+query)
...
class STTransformerBlock(nn.Module):
    ...
    #     x_fft = torch.fft.irfft(x_rttf, n=T, dim=2, norm='ortho')
    #     hidden_states = self.dropout(x_fft)
    #     x2 = self.norm3(hidden_states + query1)
    #####

        return x2
    # teacher 去掉多头注意力
    #     x_no_atten = self.norm1(self.ES_GCN(value,key, query)+query)
    #     return x_no_atten
...
#         x = x.unsqueeze(0)
        ...
        # input_Transformer = self.dropout(input_Transformer)
